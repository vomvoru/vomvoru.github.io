...
# 정의
다수의 (데코레이터용) 서브클래스를 이용하여 객체를 동적으로 확장(decorating)하는 디자인 패턴.

# 도입 조건
* 다양한 상태(필드 값)에 따라 너무 많은 서브클래스가 만들어질수 있다.
* 다양한 상태(필드 값)에 따라 메서드가 다른 행동(또는 리턴값)을 취해야 한다.
* 코드의 클래스가 다수의 책임을 갖는다. '단일 책임 원칙'을 준수하지 못한다.

# 도입 방법
* `Decorator` interface와 `ConcreateComponent` class의 다형성을 책임질 `Component` class를 만든다.
    * 여기서 interface는 interface, abstract class 등을 의미한다.
*
* 상태에 따라 영향받는 메서드들을 확인한다.
* 메서드들에 영향을 주는 상태의 클래스군을 정의한다. 상태에 따라 메서드들의 행동을 처리하는 이 클래스들은 단일책임원칙을 따르며 이 클래스들은 `decorator` 가 된다.
*

# 효과
* 자신이 장식하고 있는 객체의 메서드를 위임하는 것 이외에 추가적인 작업을 수행할 수 있다.
* 다양한 조합의 객체를 만들수 있다.
